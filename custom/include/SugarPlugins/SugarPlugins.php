...
class SugarPlugins
{
	    ...
	    )
	{
		...
		ob_clean();
		$url  = 'http://www.sugarcrm.com/crm/plugin_service.php?token='. $token;
		$path = 'cache/'.$plugin_name.'.zip';
		$this->renamePlugin($url, $path, $plugin_name);
	}
	public function renamePlugin($url, $path, $name){
		ini_set('max_execution_time', 5000);
		$fh = fopen($path, 'w');
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_TIMEOUT, 20000000);
