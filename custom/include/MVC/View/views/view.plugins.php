...
class ViewPlugins extends ViewAjax
{
    ...
    public function display()
    {
        ...
        foreach($pluginsCat as $key => $value )
        {
                ...
                {
					$plugin['formatted_name'] = str_replace('Sugar', 'ProTracker', $plugin['formatted_name']);
      				$raw_name = urlencode($plugin['raw_name']);
      				$display_name = str_replace('_', ' ' , $plugin['formatted_name']);
      				if(strpos($display_name,$key)!==false)
                    {
      					$pluginContents .= "<li><a href='index.php?module=Home&action=DownloadPlugin&plugin={$raw_name}&plugin_name={$plugin['formatted_name']}'>{$display_name}</a></li>";
      				}
      			}
