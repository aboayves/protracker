...
function rle8_decode ($str, $width){
  ...
  for ($i = 0; $i <$cnt; $i++) {
    ...
    switch ($o){
      case 0:         $i++;
        switch (ord($str[$i])){
          case 0:             $padCnt = $lineWidth - strlen($out)%$lineWidth;
            if ($padCnt<$lineWidth) $out .= str_repeat(chr(0), $padCnt);             break;
          case 1:             $padCnt = $lineWidth - strlen($out)%$lineWidth;
            if ($padCnt<$lineWidth) $out .= str_repeat(chr(0), $padCnt);             break 3;
          case 2:             $i += 2;
            break;
          default:             $num = ord($str[$i]);
            for ($j = 0; $j < $num; $j++)
              $out .= $str[++$i];
            if ($num % 2) $i++;
        }
...
function rle4_decode ($str, $width) {
  ...
  for ($i = 0; $i < $cnt; $i++) {
    ...
    switch ($o) {
      case 0:         $i++;
        switch (ord($str[$i])){
          case 0:             while (count($pixels)%$lineWidth!=0)
              $pixels[]=0;
            break;
          case 1:             while (count($pixels)%$lineWidth!=0)
              $pixels[]=0;
            break 3;
          case 2:             $i += 2;
            break;
