...
window.onbeforeunload=null;scroll(0,0);SUGAR.ajaxUI.hideLoadingPanel();SUGAR.forms.AssignmentHandler.reset();try{var r=YAHOO.lang.JSON.parse(o.responseText);cont=r.content;if(r.moduleList)
...
{DCMenu.module=r.menu.module;var hl=document.getElementById("help_link");hl.href=hl.href.replace(new RegExp("help_action=([^\&]*?)"),'help_action='+action_sugar_grp1).replace(new RegExp("help_module=([^\&]*?)"),'help_module='+r.menu.module);}
