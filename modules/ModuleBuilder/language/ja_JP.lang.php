...
$mod_strings = array (
  ...
  'LBL_CONFIRM_FIELD_DELETE' => 'カスタムフィールドを削除するとデータベース上のカスタムフィールドに関連付けられたデータもすべて削除されます。フィールドはモジュールレイアウトから削除されます。このフィールドが他の計算フィールドで使用されている場合、計算フィールドも動作しなくなります。\\n\\nまた、レポートでもこのフィールドが使用できなくなります。この変更は、一旦アプリケーションをログアウトして際ログインすると有効になります。このフィールドを含むレポートを使用したい場合はレポートを編集する必要があります。\\n\\n継続してよいですか。',
  ...
  array (
    ...
    array (
      ...
      'modify' => '<b>パッケージ</b>のプロパティと可能なアクションはここに表示されています。<br><br>パッケージの<b>名称</b>、<b>作成者</b>、<b>詳細</b>は編集可能で、パッケージに含まれるモジュールのカスタマイズはすべ閲覧することが可能です。<br><br>パッケージを作成するには<b>新規モジュール</b>をクリックしてください。<br><br>パッケージにモジュールが含まれている場合、パッケージの<b>公開</b>、<b>非公開</b>、<b>エクスポート</b>を実施することができます。',
      'name' => 'パッケージの<b>名称</b>です。<br/><br/>パッケージ名に指定できる文字列はアルファベットのみとなります（例: HR_Management）。',
      'author' => '<b>作成者</b>はパッケージ提供者としてインストール時に表示される情報となります。作成者は個人名、企業名のどちらでもかまいません。',
      'description' => 'インストール時に表示されるパッケージの<b>詳細</b>となります。',
      'publishbtn' => '内容を保存しインストール可能な.zipファイルを作成するには<b>公開</b>をクリックしてください。<br/><br/><b>モジュールローダー</b>から.zipファイルのアップロードとインストールを行うことができます。',
      'deploybtn' => 'パッケージに含まれるすべてのモジュールを保存しシステムにインストールするには<b>配置</b>をクリックしてください。',
      'duplicatebtn' => '現在のパッケージの内容をコピーして新しいパッケージを作成するには<b>複製</b>をクリックしてください。<br/><br/>新しいパッケージの名前は、システムによってコピー元パッケージの名前の末尾に数値が追加された名称が与えられます。この名前はパッケージの<b>パッケージ名</b>を変更し<b>保存</b>をクリックすることで変更することができます。',
    ...
    array (
      ...
      'DashletListViewBtn' => '<b>ダッシュレット一覧画面</b>はダッシュレット検索フィルターに基づいてレコードを表示します。',
  ...
  array (
    ...
    'listViewEditor' => 
    array (
      'modify' => '左のパネルには３つのカラムが表示されています。デフォルトカラムは一覧画面にデフォルトで表示されるフィールドを示しています。利用可能なフィールドはカスタムの一覧画面を作成する際に選択できるフィールドを示しています。非表示カラムは管理者だけがデフォルトカラムまたは利用可能カラムに移動できるフィールドです。',
