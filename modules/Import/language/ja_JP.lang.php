...
$mod_strings = array (
  ...
  'LBL_IMPORT_MODULE_ERROR_LARGE_FILE_END' => 'バイトです。config.phpのアップロードサイズの $upload_maxsize の設定を変更してください。',
  ...
  'LBL_MICROSOFT_OUTLOOK_HELP' => 'Microsoft Outlook用のカスタムマッピングはカンマ区切りのファイル（.csv）により行なうことができます。インポートファイルがタブ区切りの場合、意図したようにマッピングされません。',
  ...
  'LBL_ENCLOSURE_HELP' => '<p><b>フィールドを囲う文字</b>はフィールドの中身を囲うために利用されます。このフィールドを囲う文字の間であれば、区切り文字として利用される文字を含めることもできます。<br><br>例: 区切り文字がカンマ(,)でフィールドを囲う文字がクオーテーションマーク(")なら<br><b>"Cupertino, California"</b>は１つのフィールドとしてアプリケーションにインポートされ、<b>Cupertino, California</b>と表示されます。<br>もしフィールドを囲う文字がない場合やその他の文字なら、<br><b>"Cupertino, California"</b>は二つの隣接したフィールドとしてインポートされ、<b>"Cupertino</b>と<b>California"</b>になります。<br><br>メモ: インポートファイルには任意のフィールドを囲う文字が含まれます。<br>Excelで生成されるカンマ区切りおよびタブ区切りのファイルのデフォルトのフィールドを囲う文字列はクオーテーションマークです。</p>',
  ...
  'LBL_MIME_TYPE_ERROR_1' => '選択されたファイルには区切り文字がないようです。ファイルタイプを確認してください。推奨はカンマ区切りファイル（.csv）です。',
  ...
  'LBL_FILE_UPLOAD_WIDGET_HELP' => 'カンマ区切りやタブ区切りなどの区切り文字で区切られたデータが含まれているファイルを選択してください。推奨はカンマ区切りファイル（.csv）です。',
  ...
  'LBL_EXTERNAL_MAP_SUB_HELP' => 'インポートを開始するには<b>インポート</b>をクリックしてください。姓を含むレコードのみが作成されます。名前やメールアドレスが既存レコードとマッチする場合は重複と判断され、データは作成されません。',
