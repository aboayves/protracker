...
$mod_strings = array (
  ...
  'ERR_CHECKSYS_CONFIG_NOT_WRITABLE' => 'configファイルは存在しますが、書き込みができません。ファイルに書き込み権限を与えてください。OSに拠りますが、chmod 766などのパーミッションを変更するコマンドを実行したり、当該ファイルのプロパティで読み取り専用のチェックをはずす必要があります。',
  'ERR_CHECKSYS_CONFIG_OVERRIDE_NOT_WRITABLE' => 'config overrideファイルは存在しますが、書き込みができません。ファイルに書き込み権限を与えてください。OSに拠りますが、chmod 766などのパーミッションを変更するコマンドを実行したり、当該ファイルのプロパティで読み取り専用のチェックをはずす必要があります。',
  'ERR_CHECKSYS_CUSTOM_NOT_WRITABLE' => 'customディレクトリが存在しますが、書き込みができません。OSに拠りますが、chmod 766などのパーミッションを変更するコマンドを実行したり、当該ファイルのプロパティで読み取り専用のチェックをはずす必要があります。書き込みが可能になるように処理をしてください。',
  ...
  'REQUIRED_SYS_COMP_MSG' => '開始する前に下記の必要なコンポーネントについて正しいバージョンがインストールされているか確認してください:<br><ul><br /><li> データベース/データベース管理システム（例: MySQL、SQL Server、Oracle）</li><br /><li> Webサーバ（例: Apache、IIS）</li>                     </ul><br />サポート対象のバージョンについてはリリースメモのサポートプラットフォーム一覧を参照ください。<br>',
  'REQUIRED_SYS_CHK' => '初期のシステムチェック',
  'REQUIRED_SYS_CHK_MSG' => 'インストールを開始すると、システムはSugarCRMファイルが存在するWebサーバ上でチェックを開始し、システムが適切に構成され必要なコンポーネントがインストールされていることを確認します。<br><br><br />システムは以下をチェックします:<br><ul><br /><li><b>PHPバージョン</b> - アプリケーションがサポートするPHPバージョンが必要です。</li><br /><li><b>セッション変数</b> - 正常に動作する必要があります。</li><br /><li> <b>MB Strings</b> - エクステンションがインストールされている必要があります。</li><br /><li> <b>データベースサポート</b> - MySQL、SQL                      ServerまたはOracleのサポートが必要です。</li><br /><li> <b>Config.php</b> - このファイルが存在し、書き込みが可能である必要があります。</li><br /><li>以下のディレクトリまたはファイルが書き込み可である必要があります。:<b><ul><br /><li>/custom</li><br /><li>/cache</li><br /><li>/modules</ul></b></li></ul><br />チェックでエラーが出た場合、次には進めず、チェックが完了しなかったというエラーメッセージが表示されます。必要な修復をした後、再度チェックをすることで次に進むことができます。<br><br><br />システムチェックのエラーについてはインストールガイドを参照ください。<br>',
  'REQUIRED_INSTALLTYPE' => '通常またはカスタムインストール',
  'REQUIRED_INSTALLTYPE_MSG' => 'システムチェックの完了後、通常またはカスタムインストールのどちらかを選択できます。<br><br><br /><b>通常</b>および<b>カスタム</b>インストールのどちらにおいても下記が必要になります:<br><br /><ul><br /><li> <b>Sugarデータを保持するデータベースのタイプ</b> <ul><li>サポートされるデータベースタイプ: MySQL、MS SQL Server、Oracle<br><br></li></ul></li><br /><li> <b>データベースが動作するサーバのマシン（ホスト）名</b><br /><ul><li>データベースがWebサーバと同じローカルコンピュータにある場合はこの値は<i>localhost</i>となります。<br><br></li></ul></li><br /><li><b>Sugarデータを保持するデータベース名</b><br /><ul><br /><li> 既にデータベースが存在する場合、それと同じ名前を入力すると、インストール中にデータベースは削除されます。</li><br /><li> 既存のデータベースが存在しない場合、インストール処理中にこの名前で新たなデータベースが作成されます。<br><br></li><br /></ul></li><br /><li><b>データベース管理者のユーザ名とパスワード</b> <ul><li>データベース管理者はテーブルやユーザを作成し、データベースへの書き込みができる必要があります。</li><li>データベースがローカルコンピュータになく、あなたがデータベースで管理者でない場合は、データベース管理者に連絡してください。<br><br></li></ul></li><br /><li> <b>Sugarデータベースのみに使われるデータベースユーザ</b><ul><br /><li> ユーザはデータベース管理者でも構いませんし、既存のデータベースの管理者ユーザでも構いません。</li><br /><li> 新しいユーザを作成する場合は、新しいユーザ名とパスワードを入力してください。インストール処理中に作成されます。</li><br /></ul></ul><p><br /><b>カスタム</b>セットアップの場合、追加で下記が必要になります:<br><br /> <ul><br /> <li> <b>インストール後にSugarインスタンスにアクセスするためのURL </b>このURLはWebサーバのマシン名もしくはIPアドレスになります。<br><br></li><br /><li> [オプション] <b>セッションディレクトリのパス</b>共有サーバなどでセッション情報を脆弱性から守るため、セッション情報を特別なディレクトリに格納する場合に指定します。<br><br></li><br /><li> [オプション] <b>カスタムログディレクトリのパス</b>デフォルトのログディレクトリを上書きする場合に指定します。<br><br></li><br /><li> [オプション] <b>アプリケーションID</b>自動生成されるアプリケーションIDを上書きしたい場合に指定します。このIDは当インスタンスを他と区別するために用いられます。<br><br></li><br /><li>このロケールで一般的に用いられる<b>キャラクタセット</b><br><br></li></ul>より詳細な情報についてはインストールガイドをご覧ください。',
  ...
  'LBL_FTS_TABLE_TITLE' => 'フルテキスト検索設定を入力',
  'LBL_FTS_HOST' => 'ホスト',
  'LBL_FTS_PORT' => 'ポート',
  ...
  array (
    ...
    'txt_body' => 'あなたは、$contact_user_pwd_last_changed にパスワードのリセットを依頼しました。<br /><br />下記のリンクをクリックしてパスワードをリセットしてください:<br /><br />$contact_user_link_guid',
